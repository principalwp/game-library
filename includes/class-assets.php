...
final class Assets {
	 ...
	 * map (`Router::ROUTE_ACTIVITY`, `Router::ROUTE_JOIN`,
	 * `Router::ROUTE_GAMES`, `Router::ROUTE_GAME`) renders with no plugin
	 * script: `/activity/` and `/games/`(`/games/{slug}/`) are read-only,
	 * and `/join/` is a plain form post-back.
	 *
	 ...
	 * that can never match anything. `enqueue()` skips this route's scripts
	 * entirely when logged out (~31 KB across 5 requests, otherwise fetched
	 * for zero reachable behaviour on the plugin's only public, indexable
	 * member surface). A future route added to this map should get the same
	 * consideration if it also serves logged-out visitors.
	 *
	 * @var array<string,string[]>
	 */
	private const ROUTE_SCRIPTS = array(
		Router::ROUTE_MY_LIBRARY => array( 'search', 'library' ),
		Router::ROUTE_LIBRARY    => array( 'social' ),
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'game-library-tokens', $base_url . 'assets/css/tokens.css', array(), GAME_LIBRARY_VERSION );
		...
		// by plugins_loaded, well before wp_enqueue_scripts.
